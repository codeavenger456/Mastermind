classdef mastermind < matlab.apps.AppBase
    ...
    properties (Constant)
        ...
        POSSIBILITIES = mastermind.solutions(mastermind.MAX); % all possible solutions
    end
    properties(Access=private)
        answerPlayer = mastermind.generate(mastermind.MAX); % secret number for player component
        shortestPlayer = intmax; % smallest number of tries 
        possibilitiesAI = mastermind.POSSIBILITIES(1:length(mastermind.POSSIBILITIES)); % all possible solutions
    ...
    methods (Access = private)
        ...
        function ButtonRestartPushed(app, event)
            switch app.SwitchOption.Value
                case 'Player'
                    app.answerPlayer = mastermind.generate(app.MAX);
        ...
        function ButtonSubmitPushed(app, event)
            switch app.SwitchOption.Value
                ...
                case 'AI'
                    if app.SpinnerA.Value + app.SpinnerB.Value > app.MAX || (app.SpinnerA.Value == 3 && app.SpinnerB.Value == 1)
                        % cannot give hints exceeding the limit
                        if app.ButtonLanguage.Value
                           errordlg('Your hint is wrong', 'WARNING');
                        else
                            errordlg('ÿÿÿÿ', 'ÿÿ');
                        end
                    else
